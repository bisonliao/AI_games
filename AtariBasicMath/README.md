...
本项目本质上属于两层目标条件层级强化学习（goal-conditioned Hierarchical Reinforcement Learning）。整体策略可以写为：
...
其中，`s` 是当前游戏状态，`g = ANSWER(value)` 是高层产生的语义子目标，`a` 是低层输出的原始 Atari 动作。`ANSWER(value)` 对高层来说是一个动作，对低层来说则是需要实现的目标条件。

虽然当前 wrapper 把一道题定义为一个 episode，使每个 episode 的高层只决策一次，但层级结构仍然成立：一个高层动作会持续多个原始时间步，低层需要执行一串 `UP/DOWN/LEFT/RIGHT/NOOP/FIRE` 才能完成它，因此两层策略工作在不同的时间尺度并形成了明确的时间抽象。

当前宏动作也可以用 Options Framework 表达：

```text
Option ANSWER(value)
  initiation：任意处于答案编辑阶段的状态
  policy：π_low(a | s, ANSWER(value))
  termination：答案数字达到 value，并由低层执行 FIRE
```

它与常见长时程 HRL 的主要区别是高层 horizon 为 1：

```text
当前项目：高层选择一个子目标 -> 低层完成多个原始动作 -> episode 结束
常见 HRL：高层依次选择 g1 -> g2 -> g3 -> ...，每个子目标由低层执行
```

因此，本项目主要验证的是语义动作抽象、任务决策与原始动作执行的职责分解，以及高层子目标能否由目标条件低层策略可靠实现；它不直接研究同一 episode 内的多子目标规划、高层多步信用分配或失败后的高层重新规划。

此外，宏动作空间、目标编码和终止条件均由人工定义，高层和低层分别训练，组合阶段再冻结运行，而不是端到端联合学习或自动发现 option。更准确的定位是：

> 一种每个 episode 包含单个参数化语义子目标、且高低层独立训练的模块化两层 HRL 方法。

...
class BCAgent:
    def __init__(self, board_size: int, *, hidden_channels: int = 96,
        ...
        self.board_size = int(board_size); self.device = torch.device(device)
    ...
    def select_actions(self, boards: np.ndarray, current_players: np.ndarray,
        ...
        masks = np.asarray(action_masks).reshape(len(boards), -1).astype(bool)
        was_training = self.net.training; self.net.eval()
        with torch.no_grad():
            states = torch.from_numpy(encode_boards(boards, players)).to(self.device)
            logits = self.net(states).masked_fill(~torch.as_tensor(masks, device=self.device), -torch.inf)

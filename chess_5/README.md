...
二者共享同一个 Gymnasium 五子棋环境 `gomoku_env/`，但不互相导入，也不共享网络或 checkpoint。
...
├── gomoku_env/          # 两套算法共享的五子棋 Gymnasium 环境
├── tests/               # 自动化测试
...
`BC/` 是独立于 DQN 的多轮监督模仿学习方案。当前质量优先流程直接面向 9×9，通过 bootstrap 和最多六轮 DAgger 持续收集当前策略实际访问的带外局面。训练效果非常不错。
...
如果未来需要用 BC 初始化 DQN，需要额外设计兼容的网络结构和权重迁移流程。
...
- 两套算法只共享 `gomoku_env/` 棋盘环境；
- DQN 与 BC 的模型结构、损失函数和 checkpoint 格式均不相同。
